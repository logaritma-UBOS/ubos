...
with open('src/app/member/page.tsx', 'r', encoding='utf-8') as f:
    content = f.read()
...
content = re.sub(r'(activeTab === \'modul\' && \(.*?)(            </div>\n          \)})', r'\1' + modul_mlp + r'\2', content, flags=re.DOTALL)
...
"""
content = re.sub(r'(activeTab === \'affiliate\' && \(.*?)(            </div>\n          \)})', r'\1' + affiliate_mlp + r'\2', content, flags=re.DOTALL)
...
"""
content = re.sub(r'(activeTab === \'services\' && \(\s*<ServicesErrorBoundary>.*?)(              </div>\n\n            </div>\n          </ServicesErrorBoundary>\n          \)})', r'\1' + services_mlp + r'\2', content, flags=re.DOTALL)

# 4. Add Edukasi Tab entire block
edukasi_tab = """
          {/* Tab: Edukasi */}
          {activeTab === 'edukasi' && (
                ...
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col group">
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    ...
                    <div className="mt-auto">
                      ...
                      <button onClick={() => toast.info('Video tutorial sedang dalam penyusunan.')} className="w-full bg-slate-50 border border-slate-200 hover:bg-slate-100 text-slate-700 font-bold py-2 rounded-xl text-xs transition-colors flex items-center justify-center gap-1.5">
...
"""
content = re.sub(r'(          \{\/\* Tab: Affiliate \*\/)', edukasi_tab + r'\1', content)


with open('src/app/member/page.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
