  ...
  useEffect(() => {
    ...
    const checkAuth = async () => {
      ...
      const { data: merchantData } = await supabase
        .from('merchants')
        .select('nama_usaha, expired_at, created_at, trial_expires_at, status')
        .eq('user_id', user.id)
        .maybeSingle();
      ...
      if (merchantData) {
        ...
        if (diff <= 0) {
          ...
          const billingPath = `/ubos/${params.category === 'undefined' ? encodeURIComponent((merchantData.kategori_usaha || 'ritel').toLowerCase().split(' ')[0]) : params.category}/${params.slug}/billing`;
