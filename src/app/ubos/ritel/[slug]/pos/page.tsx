...
import { ShoppingCart, Store, Plus, Minus, CreditCard, ExternalLink, CheckCircle, Smartphone, Search, ScanLine, Camera, Printer, Tag, Settings2 } from 'lucide-react';
...
export default function POSRitelPage() {
    ...
    <>
      ...
      <div className="p-6 space-y-5 flex-1 overflow-y-auto hide-scrollbar">
        ...
        <div className="space-y-3">
          <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Pelanggan Tetap (Opsional)</label>
          <input 
            type="text" 
            value={customerName}
      ...
      <div className="flex flex-col md:flex-row min-h-full pb-[100px] md:pb-0 animate-in fade-in duration-500 relative bg-slate-50 font-sans">
        <div className="flex-1 md:h-[calc(100vh-64px)] md:overflow-y-auto">
          ...
          <header className="px-5 py-6 md:py-8 flex justify-between items-center z-10 relative bg-slate-50">
            ...
            <button onClick={() => router.push(`/ubos/ritel/${params.slug}`)} className="p-2.5 bg-white border border-slate-200 rounded-xl hover:bg-slate-100 transition-colors text-slate-500 shadow-sm hidden md:flex items-center gap-2 font-bold text-sm">
