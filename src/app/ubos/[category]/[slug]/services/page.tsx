      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-6 md:pt-10">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.5fr_1fr] items-center relative z-10">
            <div className="space-y-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="rounded-3xl border border-slate-200 bg-white border-slate-200 p-5">
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Kontak Admin</p>
                  <p className="mt-2 font-black text-slate-900">Admin Logaritma</p>
                  <p className="text-sm text-slate-500">Pesan layanan, konsultasi atau minta penawaran.</p>
                </div>
                <div className="rounded-3xl border border-slate-200 bg-white border-slate-200 p-5">
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-500">WhatsApp / Fonnte</p>
                  <p className="mt-2 font-black text-slate-900">+62 851-7966-0408</p>
                  <button
                    type="button"
                    onClick={() => handleWhatsAppOpen('Halo Admin Logaritma, saya mau tanya tentang layanan Ecosystem Services.')}
                    className="mt-4 inline-flex items-center gap-2 rounded-2xl btn-gradient-primary px-4 py-3 text-sm font-bold text-white transition hover:bg-emerald-400"
                  >
            ...
            <div className="rounded-[1.75rem] bg-white border-slate-200 p-6 shadow-xl shadow-slate-950/40 border border-white/10">
              <div className="flex items-center gap-4">
                <div className="grid h-14 w-14 place-items-center rounded-3xl bg-gradient-to-b from-emerald-500/20 to-slate-800 text-emerald-600 shadow-lg shadow-emerald-500/10">
                  <ShieldCheck size={28} />
                </div>
                <div>
                  <p className="text-sm uppercase tracking-[0.24em] text-slate-500">Layanan Terintegrasi</p>
                  <p className="mt-2 text-lg font-black text-slate-900">Solusi lengkap untuk iklan, cetak, hardware, dan sistem bisnis.</p>
                </div>
              </div>
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                {[
                  ...
                  <div key={item} className="rounded-3xl border border-slate-700/60 bg-slate-50/70 p-4">
                    <p className="text-sm font-semibold text-slate-600">{item}</p>
                    <p className="mt-2 text-sm text-slate-500">Dukung performa usaha Anda dengan layanan siap pakai.</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <section className="mt-10 space-y-8">
          <div className="grid gap-6 lg:grid-cols-[1fr_0.9fr]">
            <div className="rounded-3xl border border-slate-200 glass-card bg-white/90 p-6 shadow-xl shadow-slate-950/20">
              <div className="flex items-center gap-3 text-sm uppercase tracking-[0.24em] text-emerald-600 font-bold">
                <Megaphone size={18} /> Paket Meta Ads Manager
              </div>
              <h2 className="mt-4 text-2xl font-black text-slate-900">Pilihan Paket Setting Iklan Meta Ads</h2>
              <p className="mt-3 text-slate-500 leading-7">Kami bantu pasang dan optimasi iklan Meta Ads sesuai durasi kebutuhan usaha Anda.</p>
              <div className="mt-6 grid gap-4 sm:grid-cols-3">
                {META_ADS_PACKAGES.map((pkg) => (
                  <div key={pkg.name} className="group rounded-3xl border border-slate-200/90 bg-slate-50/90 p-5 transition hover:border-emerald-500/60 hover:bg-slate-900">
                    <div className="flex items-center justify-between gap-3">
                      <span className="rounded-full bg-emerald-50 text-emerald-600 px-3 py-1 text-xs font-black uppercase tracking-[0.24em] text-emerald-600">{pkg.badge}</span>
                    </div>
                    <h3 className="mt-4 text-xl font-black text-slate-900">{pkg.name}</h3>
                    <p className="mt-3 text-3xl font-black text-emerald-600">{pkg.price}</p>
                    <p className="mt-3 text-sm text-slate-500 leading-6">{pkg.description}</p>
                    <div className="mt-4 flex items-center justify-between text-xs text-slate-500">
                      <span className="rounded-full bg-slate-800/80 px-2 py-1">{pkg.duration}</span>
                      <span className="inline-flex items-center gap-1 text-emerald-600 font-bold"><CheckCircle2 size={14} /> Full Support</span>
                    </div>
                    <button
                      type="button"
                      onClick={() => handleWhatsAppOpen(`Halo Admin Logaritma, saya ingin pesan ${pkg.name} Meta Ads. Mohon infokan detail paket dan biaya.`)}
                      className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-2xl btn-gradient-primary px-4 py-3 text-sm font-bold text-white transition hover:bg-emerald-400"
                    >
            ...
            <div className="rounded-3xl border border-slate-200 glass-card bg-white/90 p-6 shadow-xl shadow-slate-950/20">
              ...
              <h2 className="mt-4 text-2xl font-black text-slate-900">Form Layanan Cetak</h2>
              <p className="mt-3 text-slate-500 leading-7">Pilih layanan stiker, spanduk, atau foto produk, lalu langsung kirim permintaan ke admin.</p>
              <form onSubmit={handleCetakSubmit} className="mt-6 space-y-4">
                  ...
                  <div className="mt-2 grid gap-2 sm:grid-cols-3">
                    {PRINT_SERVICES.map((service) => (
                      <button
                        ...
                        className={`rounded-2xl border px-4 py-3 text-left text-sm transition ${cetakType === service.value ? 'border-emerald-500 bg-emerald-50 text-emerald-600 text-slate-900' : 'border-slate-700 bg-slate-50 text-slate-600 hover:border-slate-500'}`}
                      >
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <label className="block">
                    ...
                    <input
                      ...
                      className="mt-2 w-full rounded-2xl border border-slate-700 bg-slate-50 px-4 py-3 text-slate-900 outline-none transition focus:border-emerald-500"
                    />
                  </label>
                  <label className="block">
                ...
                <button
                  type="submit"
                  className="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-sky-500 px-5 py-3 text-sm font-bold text-white transition hover:bg-sky-400"
                >
                  Pesan via WhatsApp / Fonnte <ArrowRight size={16} />
                </button>
              </form>
          ...
          <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
            <div className="rounded-3xl border border-slate-200 glass-card bg-white/90 p-6 shadow-xl shadow-slate-950/20">
              ...
              <h2 className="mt-4 text-2xl font-black text-slate-900">Katalog Hardware Kasir</h2>
              <p className="mt-3 text-slate-500 leading-7">Rekomendasi printer thermal bluetooth dan kertas kasir siap pakai, dengan link Shopee Affiliate.</p>
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                {HARDWARE_PRODUCTS.map((item) => (
                  <div key={item.name} className="rounded-3xl border border-slate-200 bg-slate-50/90 p-5 transition hover:border-amber-500/40 hover:bg-slate-900">
            ...
            <div className="rounded-3xl border border-slate-200 glass-card bg-white/90 p-6 shadow-xl shadow-slate-950/20">
              ...
              <h2 className="mt-4 text-2xl font-black text-slate-900">Form Pengajuan Custom OS</h2>
              <p className="mt-3 text-slate-500 leading-7">Ajukan solusi sistem bisnis kustom untuk operasi, laporan, dan integrasi multi-outlet.</p>
              <form onSubmit={handleCustomSubmit} className="mt-6 space-y-4">
                ...
                <label className="block">
                  ...
                  <textarea
                    ...
                    rows={5}
                    className="mt-2 w-full rounded-3xl border border-slate-700 bg-slate-50 px-4 py-3 text-slate-900 outline-none transition focus:border-emerald-500 resize-none"
                  />
                </label>
                <button
                  type="submit"
                  className="mt-2 inline-flex w-full items-center justify-center gap-2 rounded-2xl btn-gradient-primary px-5 py-3 text-sm font-bold text-white transition hover:bg-emerald-400"
                >
        ...
        <section className="mt-10 rounded-3xl border border-slate-200 glass-card bg-white/90 p-6 shadow-xl shadow-slate-950/20">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.24em] text-slate-500 font-bold">Catatan</p>
              <h2 className="mt-3 text-2xl font-black text-slate-900">Dapatkan layanan lebih cepat lewat WhatsApp / Fonnte</h2>
              <p className="mt-3 text-slate-500 leading-7">Setiap layanan sudah disiapkan untuk komunikasi langsung dengan tim admin Logaritma. Cukup pilih paket, isi form, dan kirim pesan untuk konfirmasi segera.</p>
            </div>
            <button
              type="button"
              onClick={() => handleWhatsAppOpen('Halo Admin Logaritma, saya ingin informasi lengkap tentang layanan Ecosystem Services.')} 
              className="inline-flex items-center gap-2 rounded-2xl bg-slate-800 px-5 py-3 text-sm font-bold text-slate-900 transition hover:bg-slate-700"
            >
