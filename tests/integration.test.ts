...
async function runTests() {
  ...
  // Cleanup from failed previous runs
  await prisma.user.deleteMany({ where: { email: { in: ['usera@test.com', 'userb@test.com'] } } })
  const userA = await prisma.user.create({ data: { name: 'User A', email: 'usera@test.com', passwordHash: 'hash' } })
  const userB = await prisma.user.create({ data: { name: 'User B', email: 'userb@test.com', passwordHash: 'hash' } })
  
  const bizA = await prisma.business.create({ data: { name: 'Biz A', userId: userA.id, businessType: 'FNB' } })
  const bizB = await prisma.business.create({ data: { name: 'Biz B', userId: userB.id, businessType: 'RETAIL' } })
